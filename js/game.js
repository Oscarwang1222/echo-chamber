...
const Game = {
  ...
  // 生成组合卡片集（2支持+1反对+1中立，不重复）
  generateCardSet(usedIds = []) {
  ...
  loadChapter1Cards() {
    ...
    // 生成一组卡片（2支持+1反对+1中立）
    const cardSet = this.generateCardSet(this.state.usedCardIds);
  ...
  startChapter2() {
    ...
    container.innerHTML = `${this.renderChapterProgress('chapter2')}
      <div style="width: 100%; max-width: 700px;">
        ...
        <h2>${chapter.subtitle}</h2>
        <p class="narrative" style="margin-top: 20px;">
          算法越来越了解你，推送的都是你想看的。<br>
          每次选择后，你会看到<strong>不同算法</strong>会如何模拟你的选择结果。
        </p>
  ...
  handleChapter2Choice(cardId, bias, element) {
    ...
    setTimeout(() => {
      ...
      if (this.chapter2ClickCount >= 10) {
        ...
        // 显示模拟环节
        this.showSimulation(bias);
      }
    }, 300);
  },

  // 显示模拟环节
  showSimulation(userBias, isChapter3 = false) {
    const feed = document.getElementById(`feedContainer${isChapter3 ? '3' : '2'}`);
    if (!feed) return;
    
    const chapter = isChapter3 ? '第三章' : '第二章';
    const focus = isChapter3 ? '多元视角' : '立场强化';
    
    // 模拟不同算法的反应（不显示具体选择）
    const simulationHTML = `
      <div class="simulation-panel" style="background: linear-gradient(135deg, #1a1a2e, #16213e); border-radius: 12px; padding: 20px; margin: 20px 0; border: 1px solid #333;">
        <h3 style="margin-bottom: 15px; color: var(--accent-yellow);">🔮 算法模拟</h3>
        <p style="color: var(--text-secondary); margin-bottom: 15px; font-size: 0.9rem;">
          不同的信息算法会如何处理你的选择？
          <br><span style="font-size: 0.8rem; opacity: 0.7;">${focus}</span>
        </p>
        
        <div style="display: grid; gap: 12px;">
          ${this.renderSimulationResult('迎合算法', '🎯', '#4f8cff', 70, isChapter3)}
          ${this.renderSimulationResult('挑战者', '⚡', '#ffc857', 35, isChapter3)}
          ${this.renderSimulationResult('流量野兽', '🔥', '#e94560', 30, isChapter3)}
          ${this.renderSimulationResult('公共广场', '🏛️', '#4ecca3', 30, isChapter3)}
        </div>
        
        <button class="btn btn-secondary mt-3" onclick="Game.continueToNextSet()" style="width: 100%;">
          继续下一组 →
        </button>
      </div>
    `;
    ...
    simDiv.innerHTML = simulationHTML;
    feed.appendChild(simDiv);
  ...
  startChapter3() {
    ...
    container.innerHTML = `${this.renderChapterProgress('chapter3')}
      <div style="width: 100%; max-width: 700px;">
        ...
        <h2>${chapter.subtitle}</h2>
        <p class="narrative" style="margin-top: 20px;">
          现实比想象的更复杂。强制遇到无法用单一视角解释的事件。<br>
          每次选择后，观察不同算法如何处理你的<strong>多元视角</strong>。
        </p>
